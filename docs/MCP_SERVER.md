...
| Tool | Purpose | Notes |
| --- | --- | --- |
| `search_vault_memory` | Retrieval over the index (BM25 lexical by default; vector/hybrid when an embedding provider is configured). | Returns chunks with note path, heading, **line range**, and snippet — **near-duplicates collapsed** (token overlap ≥ 0.8) so the same memory isn't returned (or token-charged) twice; no score float. Filters: `folder`, `tag`, `project`, `sinceDays`. Limit capped at 25. Never returns whole notes. |
