...
| `find_related_notes` | Link-graph neighbours of one **indexed** note: notes it links to + notes that link back. | Input: `path` (required). Links resolve by note name (Obsidian-style basename); only **indexed** notes appear (unresolved/excluded links are dropped), and an unindexed note is refused. Each direction lists at most **50** notes and reports how many more it has, so a hub/Map-of-Content note can't dump hundreds of paths. **Rate-limited** (120/min). |
