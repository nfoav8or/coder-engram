...
| `list_projects` | Project names under the projects root. | No inputs. Cheaper-looking than it is: it lists every Markdown file in the vault and scans the paths, so its cost scales with vault size (~0.5 ms at 1 000 notes, ~3.5 ms at 20 000) and is spent on the app's main thread. **Rate-limited** (60/min). |
