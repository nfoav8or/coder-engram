...
- **Files.** `chunks.json` (the chunk records), `metadata.json` (`version`, `builtAt`, `noteCount`, `chunkCount`, and `noteMtimes` — the per-note mtimes that let a reload resume the skip-unchanged fast path instead of re-deriving them from chunks, where a note that chunks to nothing leaves no trace), and `embeddings.json` (an empty shell until an embedding provider is configured: `{ "model": null, "dim": 0, "vectors": {} }`, then populated with cached vectors).
