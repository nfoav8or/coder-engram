...
**Size-adaptive persistence (0.11.x).** Both caches choose their on-disk layout by size at persist time: small vaults keep the classic single files, large vaults (past ~20k chunks/vectors, with hysteresis) shard into 256 files routed by FNV-1a (`utils/hash.ts`) so an edit rewrites ~1/256 of the corpus. The layout is recorded in the metadata/manifest — a reload reads it, never re-derives it — and layout switches blank the obsolete files to sentinels rather than deleting (the `VaultAdapter` deliberately has no destructive operation). `IndexManager.build`/`refresh` are async and yield to the host every 500 re-chunked notes so a large rebuild keeps the UI alive.
