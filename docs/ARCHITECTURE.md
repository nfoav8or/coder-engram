...
- `memory/memory-writer.ts` — `MemoryWriter` is the only component that writes memory: inbox proposals, double-gated direct writes, and the rejection ledger a discard records into.
...
- `retrieval/ranking.ts` — the shared ranking primitives: tokenize/snippet helpers, `applyFilters`, `diversifyByNote`, `dropNearDuplicates`, and **`fuseByRank`** with its `candidateDepthFor` companion. Rank fusion lives here rather than in either caller because both `HybridRetriever` (lexical + vector) and `EngramEngine.searchBatch` (query + query) need it, and a second copy is how two rankings drift apart.
- `utils/timeout.ts` — `withTimeout` (bound how long we WAIT for work that may never settle) plus the plugin's two timer primitives, `setTimer` / `clearTimer`. Obsidian asks plugins to schedule through `window.setTimeout` so a timer belongs to the window that created it and dies with a popout rather than firing into a detached document; the core and server layers must also run under Node, where `window` does not exist and every unit test lives. The helper resolves the host per call, so real windows get window-owned timers, Node gets the global, and nothing imports a host API. **`src/` contains no bare `setTimeout`/`clearTimeout` identifier** — the Node fallback reaches the global as `globalThis.setTimeout`. The three UI files that own a genuinely popout-scoped timer call `window.setTimeout` directly.
...
**Size-adaptive persistence (0.11.0).** Both caches choose their on-disk layout by size at persist time: small vaults keep the classic single files, large vaults (past ~20k chunks/vectors, with hysteresis) shard into 256 files routed by FNV-1a (`utils/sharding.ts`, shared by both caches so their layout rules cannot drift) so an edit rewrites ~1/256 of the corpus. The layout is recorded in the metadata/manifest — a reload reads it, never re-derives it — and layout switches blank the obsolete files to sentinels rather than deleting (the `VaultAdapter` deliberately has no destructive operation). `IndexManager.build`/`refresh` are async and yield to the host every 500 re-chunked notes so a large rebuild keeps the UI alive; because a pass can now be interleaved, the engine serializes `reindex`/`refresh` on one promise chain (`indexChain`, mirroring `embedChain`) so the auto-index debounce, a settings-triggered refresh, and the `reindex_vault` tool can never mutate one index concurrently.

**Vector cache — `EmbeddingStore`.** `embeddings/embedding-store.ts` owns `Index/embeddings.json` with the version-2 shape `{version, model, dim, vectors: { <chunkId>: {h: contentHash, n: norm, v: base64(f32 bytes)} }}` — vectors are little-endian `Float32Array` bytes, ~40% smaller than the old JSON number arrays and far cheaper to parse and hold on the heap (4 bytes per component instead of a boxed JS number); a version-1 file is migrated in place at load, so upgrading never re-embeds. `embedIndex` reuses vectors whose content hash is unchanged, drops removed chunks, and recomputes everything when the provider identity (`<id>:<model>`) changes; it embeds in batches, up to `embeddingConcurrency` batches in flight (default 1 — strictly sequential, safe for rate-limited endpoints), and checkpoints progress every ~1,024 newly embedded chunks so a long first pass interrupted by an Obsidian quit resumes instead of restarting. Content hashes are memoized by chunk object identity (IndexManager keeps the same chunk objects for unchanged notes across refreshes), so a pass triggered by one edited note hashes only that note's chunks, not the whole corpus — which is why the engine hands `embedIndex` the real `IndexedChunk` objects (an array snapshot, not per-call wrappers). All writes go through the `VaultAdapter`, so the cache never leaves the vault.
...
**Single inbox-format producer — `memory/pending-inbox.ts`.** This pure module parses and serializes `Memory/Inbox/pending-memory.md`. Its `renderPendingBlock` is now the *only* place the on-disk pending-block format is produced — `MemoryWriter.formatMemoryEntry` delegates to it — so proposing, reading, and re-rendering an entry round-trips (parse ⇄ render). Holding that round-trip means every optional field is gated on its **collapsed** value, not its raw one: `oneLine` can reduce a truthy input (a lone newline, whitespace) to `""`, and the parser maps an empty field back to `undefined` — so gating on the raw value wrote `Project: ` and lost the field on the next parse. A blank status is written as the literal `pending` the parser would read from it, and blank related paths are dropped rather than written as a bare `* ` bullet. That last one is the only member of the group reachable today: `add_memory` validates `relatedPaths` for type and length (`optionalStringArray`) but not for blankness, so an agent passing `[""]` put a malformed bullet into the file the user reviews, which the parser then silently dropped — the parsed view under-reporting what was on disk. `MemoryWriter` gains `readInbox`, `applyPending`, and `discardPending`; applying an entry resolves a destination memory file by type/project (`resolveApplyDestination`) and **appends** it (never overwriting, inside the memory root), then removes it from the inbox. This promotion path is exposed only through the desktop `PendingMemoryModal` (via `EngramEngine.getPendingMemory` / `applyPendingMemory` / `discardPendingMemory`) — never through the server. Every inbox read-modify-write runs inside an `InboxLock`, and the **engine** owns that lock rather than the writer: `updateSettings` rebuilds the writer on any settings change, so a chain stored on the writer starts empty and stops waiting for work the previous writer still had in flight. With the lock on the writer, a settings commit landing between two discards resurrected the first entry — the exact failure the serialization exists to prevent.

**The same hazard, generalized: an operation binds the objects it started with.** `updateSettings` is synchronous and runs *off* the engine's `indexChain`/`embedChain` — it is driven by a settings-tab blur or its debounce — and on a memory-root change it replaces `index`, `store`, `embeddingStore`, `extractionCache`, `writer`, `retriever` and `paths` outright. Meanwhile `IndexManager.build`/`refresh` yield to the event loop mid-pass (see the size-adaptive persistence note above), so an in-flight pass has real await points where that swap can land. Re-reading `this.index` after such a yield could therefore reach a *different, empty* manager: `persist()` on it returns silently, and an entire completed reindex was discarded while the log still reported success. `doReindex`, `doRefresh`, `doEmbedIndex`, and `scanAttachments` each now capture their manager/store/cache in a local at entry and use it for the whole pass, and check `this.index !== index` before writing engine-level bookkeeping or triggering follow-on work — a pass that outlived its root persists to the root it actually scanned (a rebuildable cache at an abandoned location, which is harmless) but never claims the new root is up to date on the strength of it. The chains serialize passes against *each other*; this is what protects a pass against the settings path, which is not on a chain at all.

**The rejection ledger (0.14.0) reuses the same producer.** `renderPendingBlock` and `parsePendingInbox` take the heading prefix as a parameter, so `Memory/Inbox/rejected-memory.md` is the *same* block format under a `## Rejected Memory: ` heading with a `Reason:` field — one on-disk shape, one parser, one round-trip property covering both files. Content is neutralized against **every** heading prefix rather than the one it is being written under, because a `## Rejected Memory: ` line is inert in the inbox and only becomes structural when the proposal is copied into the ledger on discard. `MemoryWriter` gains `readRejections`, `clearRejections`, and a private `recordRejection`, all under the same `InboxLock` as the inbox itself — the ledger is read on every `proposeToInbox` and rewritten by every discard, so it needs the serialization for the same reason the inbox does. Its key set is cached against the ledger's mtime exactly like the dedup cache, and for the same reason plus one more: deleting a record by hand is the documented way to undo a rejection, so a cache that outlived the file would keep suppressing a memory its owner had un-rejected. Ordering inside a discard is load-bearing in the opposite direction from apply: the **inbox is written first**, so a failed ledger write loses only feedback (reported to the caller as `recorded: false`) and can never leave a record claiming a still-pending entry was rejected.

**Summarization layer — `summarize/`.** `summarize/extractive.ts` is a pure module that splits a note into sentence-units and ranks them with two backends sharing one selection routine: lexical frequency-centrality (Luhn/centroid, always available offline) and, when an embedding provider is configured and reachable, embedding-centroid similarity with Maximal Marginal Relevance (MMR) to avoid redundant near-duplicate sentences. It is **extractive** — the result is a subset of the note's own sentences, returned verbatim in original order, with no generative backend. `EngramEngine.summarizeNote` drives it over a note's indexed chunks (`getNoteChunks`); it operates only on in-index notes (an unindexed note has no chunks and is refused), caps sentence-units at 200, and degrades to lexical selection if embedding fails. That degradation is decided on the vectors themselves, not on whether the provider call threw: `extractiveSummary` uses the embedding backend only when it is handed one non-empty, all-finite, uniform-width row per unit. A single non-finite component is not a degraded summary but an empty one — it poisons the shared centroid, so every cosine is `NaN`, so MMR's `val > bestVal` is false on the first pick and selection stops having chosen nothing, while still reporting `method: "embedding"`. The shipped providers cannot produce that (`parseVectorMatrix` rejects non-finite cells and ragged rows in the HTTP layer) and `EmbeddingStore.entriesMap` re-checks finiteness for the retrieval path; the check here is the same guarantee for the summarize path, so it does not depend on which caller supplied the vectors. It surfaces as the **Summarize Current Note** command and the `summarize_note` MCP tool.
