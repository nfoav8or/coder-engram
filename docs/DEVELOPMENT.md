...
- An Obsidian desktop install (1.13.0+) with a test vault.
...
- **Safe defaults.** New settings must default to the privacy-preserving, local-only choice, and `migrateSettings` must degrade a corrupt settings blob to defaults without throwing. "Without throwing" means at the point of *use*, not only at load: `defaultProject` was merged by the blind spread with no type coercion, so a corrupt `data.json` holding `42` or `{}` migrated cleanly and then threw a `TypeError` out of `sanitizeProjectName` the first time the user ran a project command — the guards on the path are `if (!project)`, which a truthy non-string passes. Every field a command reads must be coerced here, not just the ones with obvious parsing.
