...
- Nothing unreleased — 0.11.4 has just been cut.

**Next, in order**, per [LARGE_VAULTS.md](LARGE_VAULTS.md):

1. **Raise `minAppVersion` to 1.13.0 and delete `display()`** — authorized by the maintainer at the 0.11.4 review, deliberately held back from that release so a breaking change would not ride along with lint fixes. It drops update delivery to every user below Obsidian 1.13, so it wants its own release and probably its own minor version rather than a patch. Two things must be confirmed before the deletion: that `getSettingDefinitions()` covers all 26 controls **and** the action buttons and stats rows `display()` currently renders, and that removing the imperative tab leaves nothing unreachable on 1.13+. Getting this wrong silently removes settings from the UI, which is why it is a verification task and not a delete.
2. **P2.2 lazy chunk text** — `IndexedChunk.text` moves behind a chunk-handle accessor, with tokenize/stats caches persisted per shard so BM25 never needs the body. Resident heap per chunk drops from roughly 2 KB to 200 B, putting 1M chunks near 1 GB with vectors. This is the API-breaking step and lands alone on purpose: `text` is read by the chunker, all three retrievers, the summarizer, the MCP tools, and memory search.
3. **P3.1 IVF vector search** behind the existing `Retriever` interface, plus the **P3.2 large-vault mode** switch. Gated on a real-vault recall eval added to `npm run eval` — the spike's 43–57× speedup at ≥99.7% recall was measured on a deliberately clustered synthetic corpus, which flatters recall. LARGE_VAULTS.md also records a measured ~22 ms of a 65.7 ms vector query spent on a full sort that a bounded top-K would reclaim; that change is deliberately deferred to this step rather than made as a patch, because it reorders tied results.
4. **P2.3 Web Worker offload** for initial chunking, wherever in-Obsidian verification is available. The cooperative-yield fallback shipped in 0.11.0 already removes the renderer-freeze failure mode, so this is throughput rather than correctness.

Standing work between releases: the review loop that produced 0.10.1–0.11.4 keeps running — audit what is already shipped, fix what fails open first, and treat a stale document as an unfinished release. Nine passes in, the highest-yield technique has been pointing an adversarial review at the *previous pass's own commits*: it found a real defect in five consecutive ones.
