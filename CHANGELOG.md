...
- **Microsoft Office and LibreOffice documents are indexed too** (same opt-in setting): docx, pptx, xlsx, odt, odp, and ods. Both families are ZIP archives of XML, so extraction is a small dependency-free reader (the platform's own `DecompressionStream` does the inflating — zero added bundle weight, fully local). Word/Writer documents keep their heading structure; presentations render one `Slide N` section per slide; spreadsheets contribute sheet names and text cells (numeric cell soup is deliberately skipped). Attachments over 50 MB are skipped.
