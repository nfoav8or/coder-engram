...
- **Sections now chunk at 2400 characters instead of 1200.** Measured at production scale (2000 notes), the larger budget cuts the corpus from 19,132 chunks to 13,286 (−31%), the index from 18.0 MB to 15.7 MB, and lexical p50 latency from 15.8 ms to 10.0 ms (−37%), with correspondingly fewer, larger units to embed. 2400 is the knee of the curve rather than the maximum — 4000 buys only −41% chunks while making every hit a coarser passage. Search cost to the agent is unchanged: snippets are a fixed 220 characters and a result page measured 335 characters at every value tested. **Existing indexes rebuild automatically** (`INDEX_VERSION` bumped), since chunk boundaries changed.
