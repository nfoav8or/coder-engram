...
- **0.10.1 – 0.10.3 (current):** more of the same auditing. The one that matters most: everything this plugin writes into your vault and reads back later — the three caches under `Index/` and the review inbox — is now treated as something that may have changed in between, because sync, another tool, or the plugin racing itself can change it. Also a tool error that named the vault's absolute path.
